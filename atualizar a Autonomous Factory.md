...
| **Mudou `template/backend/` ou `template/frontend/`** | `git pull` resolve **pro próximo sistema novo** (o `createProjectMitra` clona a versão nova do template local). **Sistemas existentes NÃO recebem a mudança automaticamente** — eles ficaram com a versão do template vigente no momento em que foram criados. Pra propagar a mudança em sistema existente, num round futuro o Dev daquele sistema roda `pullFromS3Mitra` (traz o bundle deployado pro working dir), aplica o diff manual contra o template novo, e re-deploya com `deployToS3Mitra`. **Não fazer em massa** — cada sistema é uma decisão. |
| **Mudou pacote npm de `template/frontend/package.json`** | `git pull` + se for sistema novo, próximo build já pega. Pra sistema existente, mesma regra acima (pullFromS3 → adicionar dep no package.json → npm install → rebuild → deployToS3). |
